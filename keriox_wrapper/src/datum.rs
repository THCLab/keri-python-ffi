...
impl AttestationDatumId {
    pub fn new(testator_id: &str, attestation_id: &str) -> Self {
        AttestationDatumId{testator_id: testator_id.into() , attestation_id: attestation_id.into()}
    }
}
...
impl fmt::Display for AttestationDatumId {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        let str = ["did:keri:", &self.testator_id, "/attestationId/", &self.attestation_id].join("");
        write!(fmt, "{}", str)
    }
}
...
impl AttestationDatum {
    pub fn attach_signature(&self, signature: Vec<u8>) -> Result<SignedAttestationDatum, Error> {
        ...
        let proof = Proof {signature: b64_signature};
        Ok(SignedAttestationDatum {at_datum: self.clone(), proof})
    }

    pub fn new(msg: &str, issuer: &str) -> Self {
        let datum = Datum {message: msg.into(), issuer: issuer.into()};
        let b64_hash = base64::encode_config(blake3::hash(&serde_json::to_vec(&datum).unwrap()).as_bytes(), URL_SAFE);
    ...
    pub fn get_datum(&self) -> Result<String, Error> {
        serde_json::to_string(&self.datum).map_err(|_e| Error::Generic("serdejson error".into()))
    }
}
...
impl SignedAttestationDatum {
    pub fn get_signature(&self) -> Result<Vec<u8>, Error> {
        base64::decode_config(self.proof.signature.clone(), URL_SAFE).map_err(|e| Error::Decode64Error(e))
    }
