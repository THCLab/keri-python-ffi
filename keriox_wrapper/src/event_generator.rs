...
pub fn make_rot(pk: &Key, nxt_pk: &Key, state: IdentifierState) -> Result<Vec<u8>, Error> {
    let key_prefix = vec![pk.derive_key_prefix()];
    let nxt_key_prefix = vec![nxt_pk.derive_key_prefix()];
...
pub fn make_ixn(payload: Option<&str>, state: IdentifierState) -> Result<Vec<u8>, Error> {
...
fn make_rct(event: EventMessage, state: IdentifierState) -> Result<Vec<u8>, Error> {
